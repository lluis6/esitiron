...
- Sustituye estas variables en ORCHESTRATOR.json (por ejemplo con `envsubst`) y
  configura `ORCHESTRATOR_CONFIG_PATH` al archivo renderizado antes de montar.
...
- Crea esos usuarios en MySQL con los privilegios adecuados antes de arrancar Orchestrator.
  Ejemplo (ajusta según tu política):
  ```sql
  CREATE DATABASE orchestrator;
  CREATE USER 'orchestrator_topology'@'%' IDENTIFIED BY '...';
  GRANT REPLICATION CLIENT, PROCESS, SHOW VIEW ON *.* TO 'orchestrator_topology'@'%';
  CREATE USER 'orchestrator'@'%' IDENTIFIED BY '...';
  GRANT ALL PRIVILEGES ON orchestrator.* TO 'orchestrator'@'%';
  ```
- Si habilitas recovery/failover automático, revisa la necesidad de privilegios adicionales (como SUPER).
